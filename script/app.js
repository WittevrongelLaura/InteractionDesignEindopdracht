...
const ShowRandom = async function(type){
	...
	if (data.Response == "False"){
		...
		//imdb rating
		let rating = Math.round(data.imdbRating /2);

		//visualisatie van runtime in balk
		let space = data.Runtime.search(" ");
		let runtime = data.Runtime.substring(0, space);
		console.log(runtime);
		...
		string += `<div class="c-card">
			...
			<div class="c-card__poster"><img class="c-card__poster--image" src="${data.Poster}" alt="Movie/Serie poster" /></div>
			<label class="c-card__title"><h1 class="c-card__title--title" for="title" name="title">${data.Title}</h1></label>
				
			<div class="c-card__info">
				<label class="c-card__year" for="year" name="year">Year: ${data.Year}</label>
				...
				<label for="runtime" name="runtime" class="c-card__runtime">Runtime: ${data.Runtime}</label>
			</div>

			<div class="c-card__moredetails">
				<label for="actors" name="actors" class="c-card__actors">Actors: ${data.Actors}</label>
				<label for="plot" name="plot" class="c-card__plot">${data.Plot}</label>
